...
Deliver a mobile-friendly personal expense tracker. Visitors can create an account and sign in with email and password. Each authenticated user manages only their own categories and expenses, records an expense with the minimum required details, finds past expenses, and sees current-month spending and category totals.
...
3. As a new user, I want starter categories, so that I can record an expense immediately.
...
7. As a user, I want to create a category, so that I can organize expenses my way.
8. As a user, I want to rename one of my categories, so that its label stays meaningful.
9. As a user, I want to delete an unused category, so that my category list stays focused.
10. As a user, I want deletion blocked when a category has expenses, so that historical expenses are never left without a category.
11. As a user, I want to add an expense with a title, amount, category, and date, so that I can capture a purchase quickly.
12. As a user, I want the expense date to default to today, so that routine entry takes less typing.
13. As a user, I want to add a short note to an expense, so that I can preserve useful context.
14. As a user, I want invalid expense details rejected clearly, so that I can correct the entry.
15. As a user, I want to edit my expenses, so that I can correct mistakes.
16. As a user, I want explicit confirmation before permanently deleting an expense, so that I do not remove one accidentally.
17. As a user, I want to see a list of only my expenses, so that my financial data remains private.
18. As a user, I want to search expenses by title or note, so that I can find a transaction quickly.
19. As a user, I want to filter expenses by date range and category, so that I can narrow the list.
20. As a user, I want the dashboard to show my current-month total, so that I can understand recent spending at a glance.
21. As a user, I want a current-month category breakdown, so that I can see where my money is going.
22. As a user, I want to see recent expenses on the dashboard, so that I can orient myself after signing in.
23. As a user, I want a default-currency change to affect only new expenses, so that historical totals remain accurate.
24. As a user, I want every expense amount to retain its recorded currency, so that later preference changes do not rewrite history.
...
- Derive the current user exclusively from the authenticated session. No request may select an expense, category, or other owner by client-supplied owner ID.
...
- Create the editable starter categories Restaurants, Food, Gas, Groceries, and Entertainment for each registered user.
- Allow a category to be deleted only when no expense references it. Reassignment occurs by editing expenses; bulk recategorization is not included.
...
- Provide CRUD operations for categories and personal expenses, focused list retrieval with text, date-range, and category filters, and a current-month dashboard summary.
...
- Cover owner isolation for every category, expense, list, search, filter, dashboard, update, and delete operation.
- Cover expense validation: required title/category/date, positive amount, valid owned category, optional note, and persisted recorded currency.
- Cover category starter data, personal ownership, rename/create behavior, and the conflict returned when deleting an in-use category.
...
- Soft deletion, restore, audit history, and bulk category reassignment.
