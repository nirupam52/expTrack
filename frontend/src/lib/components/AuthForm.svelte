<script lang="ts">
	...
	let { submitting, error, onSubmit, onModeChange }: { submitting: boolean; error: string; onSubmit: (submission: AuthSubmission) => void; onModeChange: () => void } = $props();
...
<section class="auth" aria-labelledby="welcome-title">
	...
	<form class="form-stack" onsubmit={submit}>
		...
		<label class="field">Password <input bind:value={password} type="password" autocomplete={mode === 'sign-in' ? 'current-password' : 'new-password'} minlength="12" required /></label>
